...
import type { AdminUserDto, FolderAclDto, FolderDto } from '../../../preload/index.d'
...
export default function AdminPanel({
  ...
  const usersQuery = useQuery({
    ...
    queryFn: () => window.api.admin.listUsers()
  })

  const rolesQuery = useQuery({
    queryKey: ['admin', 'roles'],
    queryFn: () => window.api.admin.listRoles()
  })

  const foldersQuery = useQuery({
    queryKey: ['admin', 'acl-folders'],
    queryFn: () => window.api.admin.listAclFolders(),
    enabled: tab === 'folders'
  })

  const aclsQuery = useQuery({
    queryKey: ['admin', 'folder-acls', selectedFolderId],
    queryFn: () => window.api.admin.listFolderAcls(selectedFolderId!),
  ...
  const setRolesMutation = useMutation({
    mutationFn: (payload: { userId: string; roleCodes: string[] }) =>
      window.api.admin.setUserRoles(payload),
    onSuccess: async (user) => {
  ...
  const disableMutation = useMutation({
    mutationFn: (payload: { userId: string; isDisabled: boolean }) =>
      window.api.admin.setUserDisabled(payload.userId, payload.isDisabled),
    onSuccess: async (user) => {
  ...
  const revokeAclMutation = useMutation({
    mutationFn: (folderAclId: string) => window.api.admin.revokeFolderAcl(folderAclId),
    onSuccess: async () => {
