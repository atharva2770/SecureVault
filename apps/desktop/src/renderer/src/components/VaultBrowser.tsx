...
import type { FileDto, FolderDto } from '../../../preload/index.d'
...
function apiMoveFile(payload: { fileId: string; targetFolderId: string }): Promise<FileDto> {
  const fn = window.api.moveFile ?? window.api.files?.moveFile
  if (typeof fn !== 'function') {
    throw new Error(
      'Move is unavailable. Fully quit SecureVault and run npm run dev again (preload was outdated).'
    )
  }
  return fn(payload)
}
...
function apiCopyFile(payload: { fileId: string; targetFolderId: string }): Promise<FileDto> {
  const fn = window.api.copyFile ?? window.api.files?.copyFile
  if (typeof fn !== 'function') {
    throw new Error(
      'Copy is unavailable. Fully quit SecureVault and run npm run dev again (preload was outdated).'
    )
  }
  return fn(payload)
}
...
export default function VaultBrowser({ username, onLocked }: VaultBrowserProps): React.JSX.Element {
  ...
  const sidebarQuery = useQuery({
    queryKey: ['sidebar'],
    queryFn: () => window.api.ensureSidebar()
  })
  ...
  const myAccessQuery = useQuery({
    queryKey: ['my-access'],
    queryFn: () => window.api.admin.getMyAccess(),
  ...
  const filesQuery = useQuery({
    ...
    queryFn: () => {
      if (selection.type === 'folder') {
        return window.api.listFiles({ folderId: selection.folderId })
      }
  ...
  const deleteFolderMutation = useMutation({
    mutationFn: (folderId: string) => window.api.deleteFolder(folderId),
  ...
  const createFolderMutation = useMutation({
    mutationFn: (payload: { name: string; parentFolderId: string }) =>
      window.api.createFolder(payload),
    onSuccess: async (folder) => {
      setNewFolderName('')
  ...
  const passwordMutation = useMutation({
    mutationFn: async (password: string) => {
      ...
      if (passwordTarget.mode === 'open') {
        return window.api.openFile(payload)
      }
      return window.api.downloadFile(payload)
    },
    onSuccess: (result) => {
      if ('savedPath' in result) {
        setStatus(`Saved original file: ${result.savedPath}`)
      } else {
  ...
  async function handleMasterLock(): Promise<void> {
    await window.api.auth.lockVault()
    onLocked()
  }
  ...
  function handleFileInput(files: FileList | null): void {
    ...
    enqueueUploads(
      Array.from(files).map((file) => ({
        sourcePath: window.api.getPathForFile(file),
    ...
    <div className="flex h-dvh flex-col overflow-hidden">
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        <aside
          ...
          onMouseMove={() => void window.api.auth.touch()}
        >
          {sidebar}
        </aside>
